...
if (cfg$run_sims && Sys.getenv("sim_run") %in% c("first", "")) {
  ...
  level_sets[["CTATE set 1"]] <- list(
    ...
    sigma = 1,
    # tau = 1,
    tau = sqrt((0.2*1)/(1-0.2)), # tau2 = ICC*sigma2/(1-ICC) %>% sqrt to get tau
    ...
    true_model = list(
      ...
      # "IT" = list(delta_s=c(0,0,0,0), gamma_j=c(0,6,6,6,6)),
      # "CTI" = list(delta_s=c(0,0,0,0), gamma_j=c(0,2,4,6,6))
      
      "IT" = list(delta_s=c(0,0,0,0,0,0,0,0,0), gamma_j=c(0,6,6,6,6,6,6,6,6,6)), # KL attempt
      "CTI" = list(delta_s=c(0,0,0,0,0,0,0,0,0), gamma_j=c(0,6,4,0,0,0,0,0,0,0)) # KL attempt
    ),
    analysis_model = list(
      "IT/ATE" = list(model="IT", target="ATE"),
      "ETI/ETATE" = list(model="ETI", target="ETATE"),
